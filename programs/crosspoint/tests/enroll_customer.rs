...
#[test]
fn enroll_customer_creates_ata_and_stats() {
    ...
    let (stats_pda, _) = Pubkey::find_program_address(
        &[b"stats", customer.pubkey().as_ref(), merchant_pda.as_ref()],
        &program_id,
    );
    ...
    let accounts = crosspoint::accounts::EnrollCustomer {
        ...
        merchant: merchant_pda,
        points_mint,
        customer_stats: stats_pda,
        customer_points_account: customer_ata,
        token_program: spl_token_2022::id(),
    ...
    let tx = Transaction::new_signed_with_payer(
        &[Instruction { program_id, accounts, data }],
        Some(&customer.pubkey()), &[&customer], svm.latest_blockhash(),
    );
    let result = svm.send_transaction(tx);
    assert!(result.is_ok(), "enroll_customer failed: {:?}", result.err());
    assert!(svm.get_account(&stats_pda).is_some());
    assert!(svm.get_account(&customer_ata).is_some());
}
