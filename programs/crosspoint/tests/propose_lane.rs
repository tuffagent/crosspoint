...
fn register_a_merchant(svm: &mut litesvm::LiteSVM, program_id: Pubkey, authority: &Keypair) -> Pubkey {
    ...
    let data = crosspoint::instruction::RegisterMerchant {
        name: "Acme".into(), symbol: "ACM".into(), uri: "".into(),
    }.data();
...
#[test]
fn propose_lane_creates_inactive_lane() {
    let (mut svm, program_id) = setup();
    let (auth_lo, m_lo, _auth_hi, m_hi) = sorted_pair(&mut svm, program_id);
    let lane = lane_pda(program_id, m_lo, m_hi);

    let accounts = crosspoint::accounts::ProposeLane {
        authority: auth_lo.pubkey(), merchant_a: m_lo, merchant_b: m_hi,
        lane, system_program: system_program::id(),
    }.to_account_metas(None);
    let data = crosspoint::instruction::ProposeLane { rate_a_to_b: 1_000_000, rate_b_to_a: 1_000_000 }.data();
    let tx = Transaction::new_signed_with_payer(
        &[Instruction { program_id, accounts, data }], Some(&auth_lo.pubkey()), &[&auth_lo], svm.latest_blockhash(),
    );
    assert!(svm.send_transaction(tx).is_ok());
}

#[test]
fn propose_lane_rejects_self_lane() {
    let (mut svm, program_id) = setup();
    let auth = new_funded_keypair(&mut svm);
    let m = register_a_merchant(&mut svm, program_id, &auth);
    let lane = lane_pda(program_id, m, m);

    let accounts = crosspoint::accounts::ProposeLane {
        authority: auth.pubkey(), merchant_a: m, merchant_b: m,
        lane, system_program: system_program::id(),
    }.to_account_metas(None);
    let data = crosspoint::instruction::ProposeLane { rate_a_to_b: 1_000_000, rate_b_to_a: 1_000_000 }.data();
    let tx = Transaction::new_signed_with_payer(
        &[Instruction { program_id, accounts, data }], Some(&auth.pubkey()), &[&auth], svm.latest_blockhash(),
    );
    assert!(svm.send_transaction(tx).is_err());
}

#[test]
fn propose_lane_rejects_unsorted_pair() {
    let (mut svm, program_id) = setup();
    let (auth_lo, m_lo, _auth_hi, m_hi) = sorted_pair(&mut svm, program_id);
    // Deliberately pass merchant_a/merchant_b in the WRONG (unsorted) order.
    let lane = lane_pda(program_id, m_lo, m_hi);

    let accounts = crosspoint::accounts::ProposeLane {
        authority: auth_lo.pubkey(), merchant_a: m_hi, merchant_b: m_lo,
        lane, system_program: system_program::id(),
    }.to_account_metas(None);
    let data = crosspoint::instruction::ProposeLane { rate_a_to_b: 1_000_000, rate_b_to_a: 1_000_000 }.data();
    let tx = Transaction::new_signed_with_payer(
